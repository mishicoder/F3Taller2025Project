...
TilesetPack* CreateTilesetsPack(const char* name, const char* filename)
{
	...
	// Verificar si la asignaci�n ha sido correcta.
	if (pack == NULL)
	{
		// Muestra el error.
	...
	while (fgets(buffer, sizeof(buffer), tilestsfile) != NULL)
	{
		...
		if (strlen(buffer) > 0 && buffer[0] != '#')
		{
			...
			if (line != NULL)
			{
				// si encuentra la palabra "@break" romper� el bucle
				...
				if (key != NULL && val != NULL)
				{
					if (strcmp(key, "ts") == 0)
					{
						...
						if (newTileset != NULL)
						{
							...
							{
								printf("Imposible asignar memoria para el tileset. El tileset no ser� agregado.\n");
								hasError += 1;
							}
						}
						else 
...
TileMap* CreateMap(const char* name, const char* filename, TilesetPack* pack)
{
	...
	for (int i = 0; i < tmap->layerCount; i++)
	{
		...
		for (int ty = 0; ty < tmap->mapHeight; ty++)
		{
			...
			{
				// Cero indica que el espacio est� vac�o.
				if (tmap->layers[i]->data[tileIndex] != 0)
				{
					Tileset* tileset = GetTileset(pack, tmap->layers[i]->data[tileIndex]);
					if (tileset != NULL)
					{
						int x = pack->tiles[tmap->layers[i]->data[tileIndex]]->x;
						int y = pack->tiles[tmap->layers[i]->data[tileIndex]]->y;
						int w = pack->tiles[tmap->layers[i]->data[tileIndex]]->width;
						int h = pack->tiles[tmap->layers[i]->data[tileIndex]]->height;
						Rectangle rectSrc = { x, y, w, h };
						Rectangle rectDst = { tx * w, ty * h, w, h };
						Vector2 origin = { 0.0f, 0.0f };
						float rotation = 0.0f;
						DrawTexturePro(tileset->texture, rectSrc, rectDst, origin, rotation, WHITE);
					}
...
/*
EL ERROR EST� EN QUE EL VALOR DE TILE_WIDTH Y TILE_HEIGHT DE TILESET EST� EN VALOR 0.
*/
